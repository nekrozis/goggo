...
// This file holds the JSON readers of the galaxy package.
//
// There is deliberately no shared reader package: a member's rule belongs to the
// code that consumes it, and the rules differ from package to package — an
// identifier accepts a number here, a byte count never accepts a string.
//
...
// that the same as a null, because 26 of the 28 document members this program
// reads make no distinction between the two.
...
// absent reports whether a member was not in the document at all.
func absent(v jsontext.Value) bool { return v.Kind() == jsontext.KindInvalid }

// tokenOf decodes the single token a bare member holds. The token, not the raw
// text, is what answers for a scalar: its String() form is the unescaped string
// value and its Int()/Float()/Bool() forms are the typed ones.
...
// identifierText reads an object identity. The API encodes one identity as a
// string or as a number and both spell the identifier; a boolean is stringified
// because the live documents have been observed to carry one and an identifier
// read must not turn a readable document into a failure. A missing member and a
// null one are "", and a container is an error rather than something to
// stringify.
//
// The value a number keeps is its LITERAL text, not a float64 round trip, so an
// identifier wider than 2^53 does not lose its low digits.
func identifierText(v jsontext.Value) (string, error) {
	switch v.Kind() {
	case jsontext.KindInvalid, jsontext.KindNull:
		return "", nil
	case jsontext.KindString, jsontext.KindNumber, jsontext.KindTrue, jsontext.KindFalse:
		...
		return tok.String(), nil
	default:
		return "", fmt.Errorf("expected an identifier, got %s", jsonKind(v))
	}
}

// stringOnly reads a member that has to be a JSON string: a missing member and
// a null one are "", and any other shape — a number, a boolean, a container —
// is an error. It is the strict counterpart of memberText, for the fields where
// anything but a string would be a protocol error rather than a value.
...
// ("12.0", "1e2"), so a value the API wrote that way is retried through the
// float form and accepted when it is whole — the rule every caller of this
// reader already relied on. An out-of-range literal is an error, never a
// saturated value.
...
// rawArray renders members as the raw text of one JSON array. It is how a list
// this package assembled is put back into a document that carries raw members.
