...
// of being modeled as one (GD5 ruling 3).
type ArtifactDeps struct {
	HTTP *httpx.Client
}

// DownloadArtifact fetches one complete URL onto destination through the
// SAME attempt loop the website download runs: retry classification, failure
// cleanup (a transport break keeps the partial, anything else removes it)
// and the server timestamp moved onto the file. There is deliberately no
// exists-skip - upstream re-downloads logos and icons every run and this
// port keeps that contract (GD5 plan 3.3). The emit sink is a no-op: an
// image is not worth a progress surface; the caller reports the outcome as
